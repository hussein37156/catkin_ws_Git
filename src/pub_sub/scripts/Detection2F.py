...
def pointcloud2_to_sl_mat(point_cloud_msg):
    ...
    # Create a sl.Mat object with the number of points and 3 channels for x, y, z
    pcloud = sl.Mat(points_array.shape[0], 1, sl.MAT_TYPE.F32_C3)

    # Copy the point cloud data into the sl.Mat
    pcloud_data = pcloud.get_data()  # Get a pointer to the underlying data
    np.copyto(pcloud_data, points_array.reshape(-1, 3))  # Copy the point cloud data
